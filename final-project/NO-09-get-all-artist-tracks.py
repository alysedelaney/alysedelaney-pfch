...
for artist_id in artist_ids:
    ...
    # print(artist_res.text)
    artist_json = artist_res.json()

    artist_name = artist_json['name']

    print(f"{artist_name} | {counter}/{search_total}")
    ...
    all_data.append(artist_track_dict)
    print(f"{len(track_ids)} tracks found.\n")
    json.dump(all_data, open('artist-track-ids.json', 'w'), indent=2)
    counter += 1


#----- OLD --------
    # #if artist has more than 50 songs, choose 50 at random
    # if len(track_ids) >= 50:
    #     print(f"Found {len(track_ids)} tracks. Choosing 50 at random...")
    #     track_ids = random.sample(track_ids, 50)
    # else:
    #     print(f"Found {len(track_ids)}. Getting data...")

    # print(f"Found {len(track_ids)} tracks. Getting data...")
    ...
    #     tracks_url = f"https://api.spotify.com/v1/tracks/{track_id}"
    #     params = {
    #         "market" : "US"
    #     }
    ...
    #     features_res = requests.get(url=features_url, headers=headers)
    #     features_json = features_res.json()

    #     tracks_data = {
    #         "track_name" : tracks_json['name'],
    #         "track_popularity" : tracks_json['popularity'],
    #         "track_preview" : tracks_json['preview_url'],
    #         "track_duration" : tracks_json['duration_ms'],
    #         "track_danceability" : features_json['danceability'],
    #         "track_energy" : features_json['energy'],
    #         "track_key" : features_json['key'],
    #         "track_loudness" : features_json['loudness'],
    #         "track_mode" : features_json['mode'],
    #         "track_speechiness" : features_json['speechiness'],
    #         "track_acousticness" : features_json['acousticness'],
    #         "track_instrumentalness" : features_json['instrumentalness'],
    #         "track_liveness" : features_json['liveness'],
    #         "track_valence" : features_json['valence'],
    #         "track_tempo" : features_json['tempo'],
    #         "track_time_signature" : features_json['time_signature']
    #     }
    #     print(f"{track_counter}: {tracks_json['name']}")
    #     track_counter += 1
    #     artist_tracks.append(tracks_data)
    #     time.sleep(.5)
    ...
    # artist_file = re.sub('[^A-Za-z0-9]+', '', artist_name).lower()
    # with open(f"data/spotify_track_data/{artist_file}.json", 'w') as write_file:
    #     json.dump(artist_data, write_file, indent=2)

    # print(f"\n----------------- {artist_name} track file created. -----------------\n")
    # counter += 1
